...
function AdminProducts() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return productList.filter((product) => {
      ...
      const matchesSearch = !term || Object.values(product).join(" ").toLowerCase().includes(term);
  ...
  const openEdit = (product) => { setEditingId(product.id); setForm({ ...emptyForm, ...product }); setFormError(""); };
  const cancelForm = () => { setEditingId(null); setForm(emptyForm); setFormError(""); };

  const handleSubmit = (event) => {
    event.preventDefault();
    if (!form.name.trim() || !form.origin.trim() || !form.packaging.trim() || !form.price.trim()) {
      setFormError("Name, origin, packaging and price are required.");
      return;
    }
    const current = getProducts();
    const product = { ...form, name: form.name.trim(), origin: form.origin.trim(), packaging: form.packaging.trim(), price: form.price.trim(), description: form.description.trim() || `${form.name.trim()} prepared for wholesale seafood buyers.` };
    const next = editingId ? current.map((item) => item.id === editingId ? { ...item, ...product, id: editingId } : item) : [...current, { ...product, id: nextId("PRD", current) }];
    saveProducts(next);
    setProductList(next);
    cancelForm();
  };
  ...
  const handleDelete = (product) => {
    ...
    const next = getProducts().filter((item) => item.id !== product.id);
    saveProducts(next);
    setProductList(next);
  };
